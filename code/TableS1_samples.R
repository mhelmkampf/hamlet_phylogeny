### ============================================================================
### Hamlet phylogeny: R code to compile Table S1 (List of samples with metadata)
### Created Apr 2024 by Martin Helmkampf
### ============================================================================
...
gulf <- read_tsv(file = "/Users/martin/Documents/Projects/5_Phylogeny/samples/Collaborators/Gulf_samples.tsv") %>%
  mutate(SampleID = as.character(SampleID)) %>%
  select(SampleID, Date)

# Join all collection data
comb <- coll %>%
  ...
  full_join(., gulf, join_by(SampleID)) %>%
  mutate(Date = coalesce(Date.x, Date.y)) %>%
  select(-Date.x)
...
### Localidad / Estodo for Gulf samples
missing <- read_tsv(file = "/Users/martin/Documents/Projects/5_Phylogeny/samples/Collaborators/Gulf_samples.tsv") %>%
  mutate(SampleID = as.character(SampleID)) %>%
  right_join(., no_meta_any, join_by(SampleID)) %>%
  select(SampleID, Species, Localidad, Location, Date.x)
...
write_tsv(missing, file = "../samples/Collaborators/missing.tsv", col_names = TRUE)
