...
# RUN
runslow = TRUE
resfn = "Hamlets_DEC_TimeStratified_scheme2.Rdata"
if (runslow)
{
...
resfn = "Hamlets_DEC+J_TimeStratified_scheme2.Rdata"
runslow = TRUE
if (runslow)
{
...
# RUN
runslow = TRUE
resfn = "Hamlets_DIVALIKE_TimeStratified_scheme2.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
# RUN
resfn = "Hamlets_DIVALIKE+J_TimeStratified_scheme2.Rdata"
runslow = TRUE
if (runslow)
{
...
# RUN
runslow = TRUE
resfn = "Hamlets_BAYAREALIKE_TimeStratified_scheme2.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
# RUN
resfn = "Hamlets_BAYAREALIKE+J_TimeStratified_scheme2.Rdata"
runslow = TRUE
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
...
write.table(conditional_format_table(restable_AICc_rellike), file="Hamlets_restable_AICc_formatted_scheme2.txt", quote=FALSE, sep="\t")
...
load("Hamlets_BAYAREALIKE+J_scheme2.Rdata") #the best supported model
...
relprobs_matrix2<-as.data.frame(relprobs_matrix)
relprobs_matrix2[,9]<-1:115
colnames(relprobs_matrix2)<-c("non","C","A","G","X","X1","X2","X3","node")
#write.csv(relprobs_matrix2,file="relprobs_matrix2.csv")
only_internal_nodes<-relprobs_matrix2[-c(1:58),]
...
plt_pies<- chr + theme_tree2() + scale_x_continuous(breaks=seq(-3,0,0.5), labels=seq(3,0,-0.5),limits=c(-3,2)) +
  scale_y_continuous(limits=c(0,70)) + geom_inset(pies, width = .1, height = .1) +
...
pdf(file="Hamlets_BioGeo_BAYAREALIKE+J_scheme2_pies.pdf", height = 9, width = 8, useDingbats = FALSE)
fig_pies
dev.off()
...
only_tips<-relprobs_matrix2[c(1:58),]
only_tips[,10]<-phylo.ham$tip.label
#write.csv(only_tips,file="only_tips.csv")
relprobs_matrix2_table<-read.csv(file="relprobs_matrix2_table.csv")
...
fig2<- chr %<+% relprobs_matrix2_table + geom_tippoint(aes(color=Species),size=1)+
...
pdf(file="Hamlets_scheme2_species.pdf", height = 9, width = 8, useDingbats = FALSE)
fig2
dev.off()
...
#
#
#pdf(file="Hamlets_BioGeo_BAYAREALIKE+J_scheme2_mostprobablestate.pdf", height = 9, width = 8, useDingbats = FALSE)
#fig
#dev.off()